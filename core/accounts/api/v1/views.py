...
class RegistrationApiView(generics.GenericAPIView):
    ...
    def post (self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            email_obj = EmailMessage('email/activation_email.tpl', {'token': token}, 'admin@email.com',
                                    to=[email])
...
class CustomDiskardAuthToken(APIView):
...
class TestEmailSend(generics.GenericAPIView): 
    ...
    def get(self, request, *args, **kwargs):
        self.email = "testuser3@email.com"
        user_obj = get_object_or_404(User, email = self.email)
        token = self.get_tokens_for_user(user_obj)
        email_obj = EmailMessage('email/hello.tpl', {'token': token}, 'admin@email.com',
                               to=[user_obj.email])
        EmailThread(email_obj).start()
...
class ActivationResendAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = self.get_tokens_for_user(user_obj)
        email_obj = EmailMessage('email/activation_email.tpl', {'token': token}, 'admin@email.com', to=[user_obj.email])
        EmailThread(email_obj).start()
        return Response({'details':'Your activation token has been resended successfully.'})
